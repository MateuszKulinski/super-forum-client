...
const Thread = () => {
    ...
    const onClickPost = async (
        ...
        e.preventDefault();
        if (!userId || userId === "0") {
            setPostMsg("Użytkownim musi być zalogowany, aby dodać wątek");
        } else if (!category) {
            setPostMsg("Proszę wybrać kategorię");
        } else if (!title) {
            setPostMsg("Proszę podać tytuł");
        } else if (!bodyNode) {
            setPostMsg("Proszę podać treść");
        } else {
            setPostMsg("");
            const newThread = {
                userId,
            ...
            if (
                ...
                setPostMsg("Wątek został utworzony");
                history(`/thread/${createThreadMsg.createThread.messages[0]}`);
            } else {
